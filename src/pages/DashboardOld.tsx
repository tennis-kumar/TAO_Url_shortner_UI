...
const Dashboard = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (editingUrl) {
      ...
      const updatedData: UpdateUrlData = {};
      ...
      try {
        ...
        const data: { shortUrl: string; message?: string } = await response.json();
              ...
              urls.map((url) => (
                <tr key={url.shortUrl} className="border-t">
                  <td className="border p-2">
                    <a href={`http://localhost:5000/api/shorten/${url.shortUrl}?t=${Date.now()}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline hover:text-blue-800 hover:scale-105 transition duration-150 cursor-pointer">
                      {url.shortUrl}
                    </a>
                  </td>
